...
function retrieveArticles(query, topN = 3) {
  ...
  const scored = knowledgeBase.map(article => {
    ...
    if (queryTokens.length > 0) {
      ...
      const articleTokens = new Set([...keywordTokens, ...titleTokens]);
...
app.post('/api/query', async (req, res) => {
  ...
  if (isUnsafe) {
    warning =
      `⚠️ Health advisory: detected risk factors (${risks.join(', ')}). Consult a medical professional before attempting yoga practices.`;
  }

  let articles = retrieveArticles(q);

  if (isUnsafe && articles.length === 0) {
    articles = knowledgeBase.filter(a => a.safetyNote).slice(0,2);
  }
...
async function logQuery(question,isUnsafe,matchedArticles){
  ...
  try{
    ...
    console.warn("Query logging failed:",err.message);
  }
}
